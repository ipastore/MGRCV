...
float exponential (float base, int exponente){
    float res = 1; 
...
float arcsin(float x, int iterations){
    ...
    for (int n=0; n<iterations; n++){
        float numerator = factorial(2*n) * exponential(x,(2*n+1));
        float denominator = exponential(4,n) * exponential(factorial(n),2) * (2*n+1);
