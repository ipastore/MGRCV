...
from utils.SIFT_RANSAC_Superglue import *
...
if __name__ == '__main__':
    ...
    for i, pair in enumerate(pairs):
        ...
        if do_match:
            ...
            print(f"Processing {stem0}_{stem1}")
        ...
        if len(mkpts0) >= 8:  
            ...
            x1_superglue = np.vstack((mkpts0.T, np.ones((1, mkpts0.shape[0]))))
            x2_superglue = np.vstack((mkpts1.T, np.ones((1, mkpts1.shape[0]))))

            F, inlier_mask = ransac_fundamental_matrix(
                x1_superglue, x2_superglue, p=p, threshold=ransacReprojThreshold, confidence=confidence
                )
            # print("F matrix: ", F)
            ...
            if inlier_mask is None:
                print("No inliers found for RANSAC")
