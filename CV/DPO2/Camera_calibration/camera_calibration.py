...
if __name__ == '__main__':
    ...
    # Define el nombre de la subcarpeta que quieres usar
    subcarpeta = 'Calibration_12MP'
    images = glob.glob(os.path.join(subcarpeta, 'calib_*.jpg'))
    ...
    # Guardar la matriz K (matriz intrínseca)
    output_filename = f'./{subcarpeta}/K_{subcarpeta}.txt'
    with open(output_filename, 'w') as f:
        for row in mtx:
            f.write(" ".join([f"{val:.18e}" for val in row]) + "\n")
    print(f'Matriz K guardada como: {output_filename}')
