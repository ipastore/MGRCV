...
# Extract rotation vector (theta) and translation vector (t) from Op
    ...
    t = Op[3:6]                 # Translation vector
    X = Op[6:].reshape(-1, 3)  # Reshape 3D points to (nPoints, 3)
    ...
    x1_projected = project_to_camera(X_h, P1)
    x2_projected = project_to_camera(X_h, P2)
...
def resBundleProjection3Views12DoF(Op, x1Data, x2Data, x3Data, K_c, nPoints):
    ...
    X = Op[12:].reshape(-1, 3)      # 3D points in the reference frame of Camera 1
    ...
    x1_projected = project_to_camera(X_h, P1)
    x2_projected = project_to_camera(X_h, P2)
    x3_projected = project_to_camera(X_h, P3)
...
def visualize_residuals(image, x_data, x_proj, title):
    plt.imshow(image, cmap='gray', vmin=0, vmax=255)
    plotResidual(x_data, x_proj, 'k-')
    plt.plot(x_proj[0, :], x_proj[1, :], 'bo', label='Projected Points')
    plt.plot(x_data[0, :], x_data[1, :], 'rx', label='Observed Points')
    plotNumberedImagePoints(x_data[0:2, :], 'r', 4)
    plt.title(title)
    plt.legend()
    plt.show()
...
def visualize_3D_comparison(ax, T_w_c1, T_w_c2, X_w, T_c2_c1_initial, X_c1_w_initial, T_c2_c1_opt, X_c1_w_opt):
    """
    ...
    drawRefSystem(ax, np.eye(4), '-', 'World')
    drawRefSystem(ax, T_w_c1, '-', 'C1 (Ground Truth)')
    drawRefSystem(ax, T_w_c2, '-', 'C2 (Ground Truth)')
    ax.scatter(X_w[0, :], X_w[1, :], X_w[2, :], marker='o', color='g', label='3D Points (Ground Truth)')

    # Initial Guess (Camera 1 Frame)
    drawRefSystem(ax, np.eye(4), '--', 'C1 (Initial)')
    ...
    drawRefSystem(ax, np.eye(4), '-.', 'C1 (Optimized)')
...
def visualize_3D_3cameras_optimization(T_w_c1, T_w_c2, T_w_c3, X_w, T_c2_c1_initial, T_c3_c1_initial, T_c2_c1_opt, T_c3_c1_opt, X_initial, X_opt_scaled):
    """
    ...
    T_c2_c1_gt = T_c1_w @ T_w_c2  # Ground truth transformation of Camera 2 in Camera 1's frame
    T_c3_c1_gt = T_c1_w @ T_w_c3  # Ground truth transformation of Camera 3 in Camera 1's frame
    X_c1_gt = T_c1_w @ X_w        # Transform ground truth points into Camera 1's frame
    fig = plt.figure()
    ...
    # drawRefSystem(ax, T_w_c1, '-', 'C1 (Ground Truth)')
    drawRefSystem(ax, T_c2_c1_gt, '-', 'C2 (Ground Truth)')
    drawRefSystem(ax, T_c3_c1_gt, '-', 'C3 (Ground Truth)')
    ax.scatter(X_c1_gt[0, :], X_c1_gt[1, :], X_c1_gt[2, :], marker='o', color='g', label='3D Points (Ground Truth)')

    # Initial guess points and cameras
    drawRefSystem(ax, np.eye(4), '--', 'C1 (Initial)')
    drawRefSystem(ax, T_c2_c1_initial, '--', 'C2 (Initial)')
    drawRefSystem(ax, T_c3_c1_initial, '--', 'C3 (Initial)')
    ax.scatter(X_initial[0, :], X_initial[1, :], X_initial[2, :], marker='^', color='b', label='3D Points (Initial)')

    # Optimized points and cameras
    drawRefSystem(ax, np.eye(4), '-.', 'C1 (Optimized)')
    drawRefSystem(ax, T_c2_c1_opt, '-.', 'C2 (Optimized)')
    drawRefSystem(ax, T_c3_c1_opt, '-.', 'C3 (Optimized)')
    ax.scatter(X_opt_scaled[0, :], X_opt_scaled[1, :], X_opt_scaled[2, :], marker='x', color='r', label='3D Points (Optimized)')
...
if __name__ == '__main__':
    ...
    if excercise2:
        ...
        P3 = get_projection_matrix(K_C, T_c3_w)


        x1_proj = project_to_camera(X_w, P1)
        x2_proj = project_to_camera(X_w, P2)
        x3_proj = project_to_camera(X_w, P3)

        # Visualize initial residuals for both views
        visualize_residuals(image1, x1_data, x1_proj, 'Residuals in Image 1')
        visualize_residuals(image2, x2_data, x2_proj, 'Residuals in Image 2')
        visualize_residuals(image3, x3_data, x3_proj, 'Residuals in Image 3')
        ...
        P_c1_w_initial = get_projection_matrix(K_C, np.eye(4))
        x1_proj_initial = project_to_camera(X_c1_w_initial,P_c1_w_initial)
        visualize_residuals(image1, x1_data, x1_proj_initial, "Initial Residuals in Image 1")
        ...
        x2_proj_initial = project_to_camera(X_c1_w_initial, P_c2_c1_initial)
        visualize_residuals(image2, x2_data, x2_proj_initial, 'Initial Residuals in Image 2')
        ...
        # t_c2_c1_initial = np.array([0.1, 0.1, 0.1])
        # theta_initial = np.array([0.1, 0.1, 0.1])
        ...
        optimized = least_squares(resBundleProjection, intial_guess, args=(x1_data, x2_data, K_C, nPoints),method='trf', jac='3-point', loss='huber')

        # Extract optimized parameters
        theta_c2_c1_opt = optimized.x[:3]
        t_c2_c1_opt = optimized.x[3:6]
        X_c1_w_opt = optimized.x[6:].reshape(-1, 3)
        ...
        x1_proj_opt = project_to_camera(X_c1_w_opt_h, P_c1_w_initial)
        x2_proj_opt = project_to_camera(X_c1_w_opt_h, P_c2_c1_opt)

        # View residuals from optimized calculations
        #From World to image1
        visualize_residuals(image1, x1_data, x1_proj_opt, "Optimized Residuals in Image 1")

        #From World to image2
        visualize_residuals(image2, x2_data, x2_proj_opt, 'Optimized Residuals in Image 2')


        # Visualize the 3D results of your solution (cameras and points) and compare them with the provided ground truth.
        #Ground truth
        visualize_3D_w_2cameras(T_w_c1, T_w_c2, X_w)
        #Initial
        visualize_3D_c1_2cameras(T_c2_c1_initial, X_c1_w_initial)
        #Optimized
        visualize_3D_c1_2cameras(T_c2_c1_opt, X_c1_w_opt_h)

        # Create a 3D plot to compare all results
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        
        # Visualize comparison of ground truth, initial, and optimized results
        visualize_3D_comparison(
        ax,
        T_w_c1, T_w_c2, X_w,             # Ground truth
        T_c2_c1_initial, X_c1_w_initial, # Initial estimate
        T_c2_c1_opt, X_c1_w_opt_h          # Optimized solution
    )

            
    if excercise3: 

        # objectPoints: (Nx3) numpy array of 3D points in the reference frame of Camera 1
        # x: 3xN matrix of homogeneous 2D points in the image from Camera 3
        # distCoeffs: np.zeros((4, 1)) since distortion is zero
        #imagePoints: The corresponding 2D points in the image from Camera 3.
        ...
        R_c3_c1_initial, _ = cv2.Rodrigues(theta_c3_c1_initial)
    ...
    if excercise4:
        ...
        if excercise4_full:
            ...
            X_c1_w_opt = result.x[12:].reshape(-1, 3)
        ...
        #To compare if necessary with X_c1_w_opt_scaled in the plot
        X_c1_w_opt_h = np.vstack((X_c1_w_opt.T, np.ones((1, X_c1_w_opt.shape[0]))))
        ...
        mean_distance_ground_truth = np.mean(np.linalg.norm(X_w[:3, :], axis=0))

        # Compute the mean Euclidean norm of the optimized points
        mean_distance_optimized = np.mean(np.linalg.norm(X_c1_w_opt[:3, :], axis=0))
        ...
        visualize_3D_3cameras_optimization(T_w_c1, T_w_c2, T_w_c3, X_w, T_c2_c1_initial, T_c3_c1_initial, T_c2_c1_opt, T_c3_c1_opt, X_c1_w_initial, X_c1_w_opt_scaled)
