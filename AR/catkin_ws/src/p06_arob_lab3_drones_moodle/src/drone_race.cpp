...
void DroneRace::getGatePositions() {
    gate_positions_.clear();

    for (const auto& gate : gates_) {
        Eigen::Vector3d pos(gate.position.x, gate.position.y, gate.position.z);
        gate_positions_.emplace_back(pos);
    }
}

void DroneRace::calculateGateVectorsAndAngles() {
    gate_next_directions_.clear();
    gate_turn_angles_.clear();

    for (size_t i = 0; i < gate_positions_.size() - 1; ++i) {
        Eigen::Vector3d direction = gate_positions_[i + 1] - gate_positions_[i];
        gate_next_directions_.emplace_back(direction.normalized());

        // Calcular ángulo entre el vector dirección y la normal actual
        Eigen::Vector3d current_normal = gate_normals_[i];
        double angle = std::acos(
            direction.normalized().dot(current_normal.normalized())
        );
        gate_turn_angles_.push_back(angle);

        ROS_INFO("Gate %lu -> %lu: Direction: [%.3f, %.3f, %.3f], Angle: %.3f radians (%.3f degrees)",
                 i, i + 1, direction.x(), direction.y(), direction.z(),
...
void DroneRace::generate_gates_vertex_(mav_trajectory_generation::Vertex::Vector &vertices){
    ...
    double angle_threshold = M_PI / 6; // Ajustable: Umbral de 60 grados
    ...
    for (size_t i = 0; i < gates_.size(); ++i) {
        ...
        Eigen::Vector3d velocity = velocity_magnitude * direction; // Ensure the type matches
...
void DroneRace::generateTrajectoryExample_() {
    ...
    std::vector<double> segment_times;
    // Here we are calculating the constraints using v_max and a_max, but we could do it manually
    segment_times = estimateSegmentTimes(vertices, vel_max, acc_max);
    cout << "Segment times = " << segment_times.size() << endl;
