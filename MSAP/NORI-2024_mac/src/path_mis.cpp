...
public:
        ...
        if (em) {
            ...
            if (!scene->rayIntersect(shadowRay)) {
                ...
                if (denominator > Epsilon) {
                    ...
                    float pdf_bsdf = bounceIntersection.mesh->getBSDF()->pdf(bsdfQR_ls);  
                    float w_em_den = pdfEmitter * emitterRecord.pdf + pdf_bsdf;
                    float w_em = 0.0f;
                    if (w_em_den > Epsilon) {
                        w_em = pdfEmitter * emitterRecord.pdf / w_em_den;
                    }
    ...
    Color3f tracePath(const Scene *scene, Sampler *sampler, const Ray3f &bouncyRay, Color3f throughput, int n_bounces, const bool prevMatIsSmooth) const {
        ...
        if (bounceIntersection.mesh->isEmitter()) {
            ...
            if(n_bounces == 0){
                return computeEmitterRadiance(bouncyRay, bounceIntersection);
            }
            ...
            if (prevMatIsSmooth){
                ...
                EmitterQueryRecord emitterRecord(bounceIntersection.p);
                p_mat_mat = bounceIntersection.mesh->getBSDF()->pdf(bsdfQR);
                p_mat_em = bounceIntersection.mesh->getEmitter()->pdf(emitterRecord);
            ...
            return throughput * w_mat * computeEmitterRadiance(bouncyRay, bounceIntersection);
